...
public final class QueryUtils {
    ...
    private static List<data> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            JSONArray newsInformation = newsArray.getJSONArray("results");

            for (int i = 0; i < newsInformation.length(); i++) {

                JSONObject currentEarthquake = newsInformation.getJSONObject(i);

                String sectionName = currentEarthquake.getString("sectionName");

                String webTitle = currentEarthquake.getString("webTitle");

                String pillarName = currentEarthquake.getString("pillarName");

                data earthquake = new data(sectionName,webTitle,pillarName);
                news.add(earthquake);
            }
